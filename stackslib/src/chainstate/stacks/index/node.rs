...
// * the 6th bit is used to indicate that a compressed non-backptr
...
/// Clear all control bits (backptr, compressed, annotation)
...
impl TriePtr {
    ...
    pub fn write_bytes_compressed<W: Write>(
        ...
        let mut encoded_id = set_compressed(self.id());
    ...
    pub fn from_bytes(bytes: &[u8], format: TriePtrFormat) -> TriePtr {
        ...
        let id = clear_inline_back_block(bytes[0]);
        let chr = bytes[1];
    ...
    pub fn from_bytes_compressed(bytes: &[u8], format: TriePtrFormat) -> TriePtr {
        ...
        let id = clear_inline_back_block(encoded_id);
        let chr = bytes[1];
    ...
    /// Pointers without a `back_block` payload use `TriePtrFormat::encoded_size_compressed()`,
    /// while pointers with either backptr or inline-annotation payload use
    /// `TriePtrFormat::encoded_size()`.
    #[inline]
...
impl TrieCodecContext {
    ...
    /// - [`TriePtrFormat::V1U32`]: compressed pointer without `back_block` payload = 6 bytes
    ///   (`id|compressed`, `chr`, `u32 ptr`); compressed pointer with payload = 10 bytes
    ///   (same + `u32 back_block`).
    /// - [`TriePtrFormat::V2U64`]: compressed pointer without `back_block` payload = 10 bytes
    ///   (`id|compressed`, `chr`, `u64 ptr`); compressed pointer with payload = 14 bytes
    ///   (same + `u32 back_block`).
    ///
    /// Therefore:
