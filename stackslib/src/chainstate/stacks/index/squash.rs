...
use crate::chainstate::stacks::index::storage::{
    SquashBoundary, SquashInfo, TrieFileStorage, TrieStorageConnection,
};
...
/// Read one `/proc/self/status` memory field in KiB.
#[cfg(target_os = "linux")]
...
/// Current and peak RSS in bytes, when available.
fn rss_snapshot() -> Option<(u64, u64)> {
    #[cfg(target_os = "linux")]
    {
        let current_kib = read_proc_status_kib("VmRSS")?;
        let peak_kib = read_proc_status_kib("VmHWM")?;
        Some((current_kib * 1024, peak_kib * 1024))
    }
...
/// Log RSS at a labeled checkpoint.
fn log_rss(label: &str, checkpoint: &str) {
    match rss_snapshot() {
        Some((current, peak)) => {
            info!(
                "[{label}] RSS @ {checkpoint}: current={}, peak={}",
                fmt_bytes(current),
                fmt_bytes(peak)
            );
        }
        None => {
            info!("[{label}] RSS @ {checkpoint}: n/a (non-Linux)");
        }
    }
}
...
fn collect_per_height_metadata<T: MarfTrieId>(
    ...
    drop(headers);
    log_rss(label, "after [2/8] drop(headers)");
...
impl<T: MarfTrieId> MARF<T> {
    ...
    fn squash_to_path_inner(
        ...
        log_rss(label, "after [1/8] block_entries");
        ...
        log_rss(label, "after [3/8] collect_trie_nodes");
        ...
        log_rss(label, "after squash complete");
    ...
    ///
    /// Returns the disk-backed store and the `(source_block_id, byte_offset) ->
    /// node index` map needed by the remap pass.
    fn collect_reachable_nodes(
        ...
        // One frame per trie level, with remaining child pointers for that node.
        struct DfsFrame {
            origin_block_id: u32,
    ...
    pub fn validate_squashed_at_height(
        ...
        let (source_block_hashes, source_root_hashes) = src.with_conn(|conn| {
            ...
            for h in 0..=height {
                ...
                let bh = MARF::<T>::get_block_at_height(conn, h, source_tip)?.ok_or_else(|| {
                    Error::CorruptionError(format!(
                        "validate_squashed_at_height: source MARF has no height->hash \
                             mapping at height {h} under tip {source_tip}"
                    ))
                })?;
                let &block_id = source_block_map.get(&bh).ok_or_else(|| {
                    Error::CorruptionError(format!(
            ...
            Ok::<_, Error>((source_block_hashes, source_root_hashes))
        })?;
        info!(
