 ...
 */
const PATTERN_OP_MAP = {
  $contains:   { sqlOp: "LIKE",     wrap: (v) => `%${v}%` },
  $ncontains:  { sqlOp: "NOT LIKE", wrap: (v) => `%${v}%` },
  $startsWith: { sqlOp: "LIKE",     wrap: (v) => `${v}%` },
  $endsWith:   { sqlOp: "LIKE",     wrap: (v) => `%${v}` },
};
 ...
 */
export function buildWhere(node) {
  ...
  if ("$and" in node) {
    ...
    const parts = node.$and.map(buildWhere);
    return {
  ...
  if ("$or" in node) {
    ...
    const parts = node.$or.map(buildWhere);
    return {
 ...
 */
export function parseFilterParam(filterStr) {
