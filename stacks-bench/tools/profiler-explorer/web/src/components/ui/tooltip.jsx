...
function TooltipProvider({ delayDuration, children, ...props }) {
  return (
...
function TooltipTrigger({ asChild, children, ...props }) {
  if (asChild) {
    return <TooltipPrimitive.Trigger render={React.Children.only(children)} {...props} />;
  }
  return <TooltipPrimitive.Trigger {...props}>{children}</TooltipPrimitive.Trigger>;
}

function TooltipContent({ className, sideOffset = 4, side, ...props }) {
  return (
